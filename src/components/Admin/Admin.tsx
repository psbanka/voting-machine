...
function Admin({ exitAdminMode }: AdminProps){
  ...
  function handleElectionReset(){
    setDoc(doc(db, 'elections', 'current'), { state: 'not-started', users: [] });
  }

  function handleStartTheElection(){
    setDoc(doc(db, 'elections', 'current'), { state: 'voting' }, { merge: true });
  }
  ...
  async function handleAddRandomVoter(){
    ...
    await setDoc(doc(db, 'votes', user.id), { finished: false, firstChoice: [], secondChoice: [], thirdChoice: [] });
    ...
    setDoc(doc(db, 'elections', 'current'), { users: [...electionData.users, user.id]}, { merge: true });
  }

  async function handleAddVotes(voterId: string){
    const candidates = await getDocs(collection(db, 'candidates'));
    const ids = shuffleArray(candidates.docs.map((doc) => doc.id));
    await setDoc(doc(db, 'votes', voterId), { finished: true, firstChoice: ids.slice(0,3), secondChoice: ids.slice(3,6), thirdChoice: ids.slice(6,9) });
  }
  ...
  return (
    <div className='admin'>
              ...
              {voters.map((voter) => (
                <div className='userInfo' key={voter.id}>
                  <div className="user">
                    ...
                    {finishedVoters.includes(voter.id) ? <p>✅</p> : <button onClick={() => handleAddVotes(voter.id)}>Add Votes</button>}
                  </div>
                </div>
              ))}
                </ul>
      ...
      <ul>
        <li><button onClick={handleElectionReset}>Reset election</button></li>
        <li><button onClick={handleAddRandomVoter}>Add random voter</button></li>
        <li><button onClick={handleStartTheElection}>Start the election</button></li>
        <li><button onClick={handleFinishElection}>Finish the election</button></li>
        <li><button onClick={exitAdminMode}>EXIT ADMIN MODE</button></li>
      </ul>
    </div>
  )
}
